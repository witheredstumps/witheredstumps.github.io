...
The classifying space of $$G$$ can then be modelled by the geometric realization of the nerve, i.e. $$BG \simeq |N(\mathbf{B} G)|$$. Note that $$BG$$ has a rogue $$0$$-simplex $$e^0$$, corresponding to the empty tuple of composable morphisms. This seems like a stupid thing to have but is very important for $$BG$$ to be a based space.
...
We move on to discuss orientability and Thom isomorphism. It is nice to do this at some level of generality. Suppose $$\pi : V \to X$$ is a real vector bundle of rank $$k$$ over a space $$X$$, and let $$E = \{E_n : n \in \mathbf{Z}\}$$ be an $$\Omega$$-spectrum with a ring structure. Then $$E$$ gives rise to a multiplicative generalized cohomology theory $$E^*$$. We shall say the vector bundle $$(V, X, \pi)$$ has a fiberwise $$E$$-orientation if there exists a choice of element $$u_x \in E^k(V_x, V_x \setminus 0_x)$$ for every $$x \in X$$ and fiber $$V_x$$ over $$x$$, such that for any two points $$x, y \in X$$ and any trivializing open set $$U \subset X$$ containing $$x$$ and $$y$$, the composition of the excision isomorphisms 
...
$$\displaystyle
\begin{CD}
\widetilde{E}^{n+k}(V_\cup, \dot{V}_\cup) @>>> \widetilde{E}^{n+k}(V_1, \dot{V}_1) \oplus \widetilde{E}^{n+k}(V_2, \dot{V}_2) @>>> \widetilde{E}^{n+k}(V_\cap, \dot{V}_\cap)\\
